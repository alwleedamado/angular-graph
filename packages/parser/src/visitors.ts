...
import { parseComponent, parsePipe, parseInjectable, parseDirective, parseNgmodule } from './parsers/angular';
...
export type AngularDecoratorType = 'NgModule' | 'Component' | 'Pipe' | 'Injectable' | 'Directive'
export function visitAngularNode(type: AngularDecoratorType, node: ts.ClassDeclaration) {
    switch (type) {
        case 'NgModule':
...
export function visitClass(node: ts.ClassDeclaration) {
    ...
    if (decorators && decorators.length > 0) {
        ...
        const type = decorator.expression['expression']['escapedText']; console.log(type)
